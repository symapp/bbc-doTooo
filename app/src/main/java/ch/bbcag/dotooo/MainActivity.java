...
public class MainActivity extends AppCompatActivity implements SearchView.OnQueryTextListener {
    ...
    private ArrayList<Task> getFormattedTaskListByDay(ArrayList<Task> allTasks) {
        ...
        if (filter_date != null) {
            ...
            String dayOfWeek = Objects.requireNonNull(filter_calendar.getDisplayName(Calendar.DAY_OF_WEEK, Calendar.LONG, Locale.getDefault())).toUpperCase(Locale.ROOT);

            tasks.add(new Task("?notTask!" + DateFormatter.format(filter_date) + "!" + dayOfWeek, "", new Date(), Color.BLACK.getDisplayName()));
        }
        ...
        if (filter_date != null && tasks.size() == 1)
            tasks.add(new Task("?notTask!No tasks", "", new Date(), Color.BLACK.getDisplayName()));
